...
class AuthorController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = DigitalAuthor::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('DigitalAuthor', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = Yii::t('phrase', 'Digital Authors');
	...
	public function actionAdd() 
	{
		...
		if(isset($_POST['DigitalAuthor'])) {
			$model->attributes=$_POST['DigitalAuthor'];
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'DigitalAuthor success created.').'</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'DigitalAuthor success created.'));
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['DigitalAuthor'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'get' => Yii::app()->controller->createUrl('manage'),
							'id' => 'partial-digital-author',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'DigitalAuthor success updated.').'</strong></div>',
						));
					} else {
						print_r($model->getErrors());
					}
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'DigitalAuthor success updated.'));
					//$this->redirect(array('view','id'=>$model->author_id));
					$this->redirect(array('manage'));
				}
			}
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				DigitalAuthor::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				DigitalAuthor::model()->deleteAll($criteria);
			}
		}
	...
	public function actionDelete($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				if($model->delete()) {
					echo CJSON::encode(array(
						...
						'get' => Yii::app()->controller->createUrl('manage'),
						'id' => 'partial-digital-author',
						'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'DigitalAuthor success deleted.').'</strong></div>',
					));
				}
			}
			...
			$this->pageTitle = Yii::t('phrase', 'DigitalAuthor Delete.');
			$this->pageDescription = '';
			$this->pageMeta = '';
	...
	{
		$model = DigitalAuthor::model()->findByPk($id);
		if($model===null)
