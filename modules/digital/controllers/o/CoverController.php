...
class CoverController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = DigitalCover::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('DigitalCover', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = Yii::t('phrase', 'Digital Covers');
	...
	public function actionAdd() 
	{
		...
		if(isset($_POST['DigitalCover'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'DigitalCover success created.').'</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'DigitalCover success created.'));
		...
		$this->pageTitle = Yii::t('phrase', 'Create Digital Covers');
		$this->pageDescription = '';
		$this->pageMeta = '';
