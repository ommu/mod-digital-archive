...
class FileController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = DigitalFile::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('DigitalFile', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = Yii::t('phrase', 'Digital Files');
	...
	public function actionAdd() 
	{
		...
		if(isset($_POST['DigitalFile'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'DigitalFile success created.').'</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'DigitalFile success created.'));
		...
		$this->pageTitle = Yii::t('phrase', 'Create Digital Files');
		$this->pageDescription = '';
		$this->pageMeta = '';
