...
class CategorytagController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = DigitalCategoryTag::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('DigitalCategoryTag', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
	...
	{
		$model=new DigitalCategoryTag;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['DigitalCategoryTag'])) {
			$model->attributes=$_POST['DigitalCategoryTag'];
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'DigitalCategoryTag success created.').'</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'DigitalCategoryTag success created.'));
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['DigitalCategoryTag'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'get' => Yii::app()->controller->createUrl('manage'),
							'id' => 'partial-digital-category-tag',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'DigitalCategoryTag success updated.').'</strong></div>',
						));
					} else {
						print_r($model->getErrors());
					}
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'DigitalCategoryTag success updated.'));
					//$this->redirect(array('view','id'=>$model->id));
					$this->redirect(array('manage'));
				}
			}
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				DigitalCategoryTag::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				DigitalCategoryTag::model()->deleteAll($criteria);
			}
		}
	...
	{
		$model = DigitalCategoryTag::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='digital-category-tag-form') {
