...
class TagController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = DigitalTag::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('DigitalTag', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = Yii::t('phrase', 'Digital Tags');
	...
	public function actionAdd() 
	{
		...
		if(isset($_POST['DigitalTag'])) {
			$model->attributes=$_POST['DigitalTag'];
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'DigitalTag success created.').'</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'DigitalTag success created.'));
		...
		$this->pageTitle = Yii::t('phrase', 'Create Digital Tags');
		$this->pageDescription = '';
		$this->pageMeta = '';
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['DigitalTag'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'get' => Yii::app()->controller->createUrl('manage'),
							'id' => 'partial-digital-tag',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'DigitalTag success updated.').'</strong></div>',
						));
					} else {
						print_r($model->getErrors());
					}
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'DigitalTag success updated.'));
					//$this->redirect(array('view','id'=>$model->id));
					$this->redirect(array('manage'));
				}
			}
		...
		$this->pageTitle = Yii::t('phrase', 'Update Digital Tags');
		$this->pageDescription = '';
		$this->pageMeta = '';
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				DigitalTag::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				DigitalTag::model()->deleteAll($criteria);
			}
		}
