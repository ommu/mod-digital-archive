...
class SiteController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = Digitals::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('Digitals', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = Yii::t('phrase', 'Digitals');
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'id' => 'partial-digitals',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'Digitals success created.').'</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					...
					//$this->redirect(array('view','id'=>$model->digital_id));
					$this->redirect(array('manage'));
				}
			}
		}
