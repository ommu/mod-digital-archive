...
class PublisherController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = DigitalPublisher::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('DigitalPublisher', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = Yii::t('phrase', 'Digital Publishers');
	...
	{
		$model=new DigitalPublisher;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['DigitalPublisher'])) {
			$model->attributes=$_POST['DigitalPublisher'];
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'DigitalPublisher success created.').'</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'DigitalPublisher success created.'));
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['DigitalPublisher'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'get' => Yii::app()->controller->createUrl('manage'),
							'id' => 'partial-digital-publisher',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'DigitalPublisher success updated.').'</strong></div>',
						));
					} else {
						print_r($model->getErrors());
					}
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'DigitalPublisher success updated.'));
					//$this->redirect(array('view','id'=>$model->publisher_id));
					$this->redirect(array('manage'));
				}
			}
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				DigitalPublisher::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				DigitalPublisher::model()->deleteAll($criteria);
			}
		}
	...
	public function actionDelete($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				if($model->delete()) {
					echo CJSON::encode(array(
						...
						'get' => Yii::app()->controller->createUrl('manage'),
						'id' => 'partial-digital-publisher',
						'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'DigitalPublisher success deleted.').'</strong></div>',
					));
				}
			}
			...
			$this->pageTitle = Yii::t('phrase', 'DigitalPublisher Delete.');
			$this->pageDescription = '';
			$this->pageMeta = '';
	...
	{
		$model = DigitalPublisher::model()->findByPk($id);
		if($model===null)
